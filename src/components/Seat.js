...
const Seat = (props) => {
    ...
    const seatClickHandler = async (event, seatNumber) => {
      try {
        if(status){
            toast.error('this is already booked please select another');
            return
           }
           const response=await axios.post(API_HOST+'/api/accounts/slot-book',{slotId:seatNumber}, {
            headers: {
...
useEffect(()=>{
    ...
    return (
        <div className="col-2 col-md-2">
            <div className={`seat ${props.seatColor} seat-${seatNumber} ${status?'seat-black':'seat-grey'}`}
